...
function canvas_arrow(context_bias, fromx, fromy, tox, toy, bias) {
	...
	context_bias.lineTo(tox - headlen * Math.cos(angle - Math.PI / 6), toy - headlen * Math.sin(angle - Math.PI / 6));
	context_bias.lineTo(tox, toy);
	context_bias.lineTo(tox - headlen * Math.cos(angle + Math.PI / 6), toy - headlen * Math.sin(angle + Math.PI / 6));
...
function drawBias(context_bias) {
	...
	canvas_arrow(context_bias, center_x, center_y, map_range(u, 1.0, -1.0, start_u_x, end_u_x), map_range(u, 1.0, -1.0, start_u_y, end_u_y), u);

	// V bias (yellow)
	reset_color(context_bias);
	context_bias.strokeStyle = color_v;
	context_bias.fillStyle = color_v;
	canvas_arrow(context_bias, center_x, center_y, map_range(v, -1.0, 1.0, start_v_x, end_v_x), map_range(v, -1.0, 1.0, start_v_y, end_v_y), v);

	// W bias (green)
	reset_color(context_bias);
	context_bias.strokeStyle = color_w;
	context_bias.fillStyle = color_w;
	canvas_arrow(context_bias, center_x, center_y, map_range(w, -1.0, 1.0, start_w_x, end_w_x), map_range(w, -1.0, 1.0, start_w_y, end_w_y), w);
